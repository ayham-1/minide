    ...
    switch (seq_len) {
        ...
        case 4:
            ...
            if (!((*first == 0xF0
                ...
                && IS_TAIL_BYTE(*second) && IS_TAIL_BYTE(*third) && IS_TAIL_BYTE(*fourth))
                || (*first == 0xF4
...
u8encode u8str_from_code_point(byte_t c[9]) {
     ...
     */
    size_t seq_len = 0;
    if (rawByte <= 0x7F) seq_len = 1;
    else if (rawByte <= 0x7FF) seq_len = 2;
    else if (rawByte <= 0xFFFF) seq_len = 3;
    else if (rawByte <= 0x1FFFFF) seq_len = 4;
    ...
    for (size_t b_index = 0; b_index < seq_len; b_index++) {
        ...
        if (b_index == 0) {
            ...
            if (seq_len == 4) a[0] |= U8_4_BYTE_MASK;
            else if (seq_len == 3) a[0] |= U8_3_BYTE_MASK;
            else if (seq_len == 2) a[0] |= U8_2_BYTE_MASK;
            else if (seq_len == 1) a[0] |= U8_1_BYTE_MASK;
        } else {
