...
int main(int argc, char *argv[]) {
    ...
    if (U_FAILURE(u_version_error_code)) {
        ...
        log_info("ICU unicode data version: %i.%i.%i.%i", u_info[0], u_info[1],
                 u_info[2], u_info[3]);
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (config.scr_target_fps &&
            glfwGetTime() < frameStartTime + 1.0 / config.scr_target_fps) {
            sleep(glfwGetTime() -
...
void __gl_callback(GLenum source, GLenum type, GLuint id, GLenum severity,
    ...
    if (GL_DEBUG_TYPE_ERROR) {
        log_error(
            "__gl_callback: %s type = 0x%x, severity = 0x%x, message = %s\n",
