...
bool cbuffer_create(cbuffer_t *cbuf, size_t capacity, size_t entrySz,
                    const char *bname) {
  ...
  cbuf->buf =
      mmap(NULL, 2 * cbuf->sz, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  void *result = NULL;
  result = mmap(cbuf->buf, cbuf->sz, PROT_READ | PROT_WRITE,
  ...
  result = mmap(cbuf->buf + cbuf->sz, byte_size, PROT_READ | PROT_WRITE,
                MAP_SHARED | MAP_FIXED, cbuf->_fd, 0);
  if (result == MAP_FAILED) {
  ...
  log_info(
      "buffer %s, allocated with %i bytes total, with entry capacity of %i",
...
bool cbuffer_get_by_index(cbuffer_t *cbuf, uint8_t *data, size_t index) {
  if (index > (cbuf->sz))
    return false;
