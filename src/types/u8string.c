...
size_t u8str_get_encode_seq(byte_t *ptr, u8encode *out) {
  ...
  if (bytes_num == 4) {
    *out =
        COMBINE_BYTES_TO_WBYTE(src_ptr[0], src_ptr[1], src_ptr[2], src_ptr[3]);
  } else if (bytes_num == 3) {
...
bool u8str_is_utf8_valid(u8encode c) {
  ...
  case 3:
    ...
    if (!((*first == 0xE0 && *second >= 0xA0 && *second <= 0xBF &&
           IS_TAIL_BYTE(*third)) ||
          (*first >= 0xE1 && *first <= 0xEC && IS_TAIL_BYTE(*second) &&
           IS_TAIL_BYTE(*third)) ||
          (*first == 0xED && *second >= 0x80 && *second <= 0x9F &&
           IS_TAIL_BYTE(*third)) ||
          (*first >= 0xEE && *first <= 0xEF && IS_TAIL_BYTE(*second) &&
           IS_TAIL_BYTE(*third))))
      return false;
    else
      break;
  case 4:
    ...
    if (!((*first == 0xF0 && *second >= 0x90 && *second <= 0xBF &&
          ...
          (*first >= 0xF1 && *first <= 0xF3 && IS_TAIL_BYTE(*second) &&
           IS_TAIL_BYTE(*third) && IS_TAIL_BYTE(*fourth)) ||
...
u8encode u8str_from_code_point(byte_t c[9]) {
  ...
  for (size_t b_index = 0; b_index < seq_len; b_index++) {
    a[b_index] =
        (rawByte >> (6 * (seq_len - b_index - 1))) & (U8_CODEPOINT_MASK);
    if (b_index == 0) {
      /* control byte */
