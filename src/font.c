...
void font_set_pixel_size(font_t *font, short pixel_size) {
     ...
     * font_t holds glyph_cache per pixel_size, however, the switch between them
     * is not done explicitly in glyph_cache but rather here. And the
     * configuration of correct size is done here by FT_Set_Pixel_Sizes() and
     * hb_font_changed()
     *
     * To correctly use this function, call before doing hb_shape() - shaper.h
     * does this.
     */
    if (FT_IS_SCALABLE(font->face)) {
        ...
        for (FT_Int i = 0; i < font->face->num_fixed_sizes; i++) {
            short new_distance = abs(font->face->available_sizes[i].height -
                                     (FT_Short)pixel_size);
...
glyph_cache *font_create_glyph_cache(font_t *font, short pixel_size) {
    if (font->caches_fullness + 1 >= font->caches_capacity) {
        ...
        font->caches = (glyph_cache *)realloc(
            font->caches, 2 * font->caches_capacity * sizeof(glyph_cache));
