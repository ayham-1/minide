 ...
 */
void rsvg_port_free(FT_Pointer * state) { free(*state); }
...
FT_Error rsvg_port_render(FT_GlyphSlot slot, FT_Pointer * _state)
{
	...
	/* `slot->bitmap.buffer`.                                           */
	surface = cairo_image_surface_create_for_data(slot->bitmap.buffer, CAIRO_FORMAT_ARGB32, (int)slot->bitmap.width,
						      (int)slot->bitmap.rows, slot->bitmap.pitch);
...
FT_Error rsvg_port_preset_slot(FT_GlyphSlot slot, FT_Bool cache, FT_Pointer * _state)
{
	...
	handle = rsvg_handle_new_from_data(document->svg_document, document->svg_document_length, &gerror);
	if (handle == NULL) {
	...
	rsvg_handle_get_intrinsic_dimensions(handle, &out_has_width, &out_width, &out_has_height, &out_height,
	...
	if (out_has_viewbox == TRUE) {
		 ...
		 */
		if (out_width.length == 1 && out_height.length == 1) {
	 ...
	 */
	state->rec_surface = cairo_recording_surface_create(CAIRO_CONTENT_COLOR_ALPHA, NULL);
	...
	x0 = (double)document->delta.x / 64 * dimension_svg.width / metrics.x_ppem;
	y0 = -(double)document->delta.y / 64 * dimension_svg.height / metrics.y_ppem;
...
#if LIBRSVG_CHECK_VERSION(2, 52, 0)
	{
		...
		ret = rsvg_handle_render_layer(handle, rec_cr, id, &viewport, NULL);
	}
#else
	...
	cairo_recording_surface_ink_extents(state->rec_surface, &x, &y, &width, &height);
	...
	vertBearingX = slot->metrics.horiBearingX / 64.0f - slot->metrics.horiAdvance / 64.0f / 2;
	vertBearingY =
	    (slot->metrics.vertAdvance / 64.0f - slot->metrics.height / 64.0f) / 2; /* XXX parentheses correct? */
...
SVG_RendererHooks rsvg_hooks = {(SVG_Lib_Init_Func)rsvg_port_init, (SVG_Lib_Free_Func)rsvg_port_free,
				(SVG_Lib_Render_Func)rsvg_port_render, (SVG_Lib_Preset_Slot_Func)rsvg_port_preset_slot};
