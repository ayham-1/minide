...
bool glyph_cache_create(glyph_cache *cache, FT_Face font_face, size_t capacity,
    ...
    hash_table_create((hash_table_t *const)&cache->table, cache->capacity,
                      __glyph_cache_table_hash, __glyph_cache_table_eql_func,
...
DATA_TYPE *glyph_cache_append(glyph_cache *cache, KEY_TYPE glyphid) {
    if (cache->fullness + 1 >= cache->capacity) {
        ...
        cache->keys = (KEY_TYPE *)realloc(cache->keys, 2 * cache->capacity *
                                                           sizeof(KEY_TYPE));
        cache->data = (DATA_TYPE *)realloc(cache->data, 2 * cache->capacity *
                                                            sizeof(DATA_TYPE));
        cache->capacity *= 2;
        ...
        for (size_t i = 0; i < cache->fullness; i++) {
            hash_table_insert(&cache->table, (void *)&cache->keys[i],
                              (void *)&cache->data[i]);
        }
    ...
    FT_Error err = FT_Load_Glyph(cache->font_face, cache->keys[cache->fullness],
                                 load_flags);
    if (err != 0) {
...
void __glyph_cache_atlas_build(glyph_cache *cache) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, cache->awidth, cache->aheight, 0,
...
void __glyph_cache_atlas_refill_gpu(glyph_cache *cache) {
    ...
    for (size_t i = 0; i < cache->table.capacity; i++) {
        ...
        while (curr_entry != NULL) {
            ...
            glTexSubImage2D(
                GL_TEXTURE_2D, 0, offset_x, offset_y,
